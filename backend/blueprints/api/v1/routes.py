import traceback, base64, io, requests, uuid, os, urllib
from flask import (
    Blueprint,
    current_app as app,
    request,
...
@api_bp.route("/speech-to-text/<engine>", methods=["POST"])
        ...
        if "audio" in request.files or test_file:
            ...
            if test_file:
                ...
                file_name = generate_random_filename(
                    app.config.get("UPLOAD_FOLDER") + "/", filename
                )
            ...
            if engine == "openai":
                ...
                return OpenAiClient.speech_to_text(
                    audio_file,
                    create_args,
                    base_url=f"{app.config.get('FAST_WHISPER_API_URL')}/v1",
                )
...
@api_bp.route("/chat/<engine>", methods=["POST"])
def chat(engine):
    ...
    try:
        ...
        if engine == "openai":
            ...
            return OpenAiClient.completion(
                ...
                create_args,
                f"{app.config.get('OLLAMA_API_URL')}/v1",
            )
            ...
            return OpenAiClient.completion(
                ...
                max_conversation_history,
                create_args,
                f"{app.config.get('LMSTUDIO_API_URL')}/v1",
            )
...
@api_bp.route("/text-to-speech/<engine>", methods=["POST"])
def text_to_speech(engine):
    ...
    try:
        ...
        create_args = {key: params[key] for key in params if not key.startswith("__")}
        file_name = generate_random_filename(
            app.config.get("UPLOAD_FOLDER") + "/", ".wav"
        )
        ...
        if engine == "openai":
            ...
            base_url = app.config.get("OPENVOICE_API_URL") + "/" + engine[-2:].lower()
            ...
            if app.config.get("ELEVENLABS_API_KEY"):
                client = ElevenLabs(api_key=app.config.get("ELEVENLABS_API_KEY"))
            else:
                client = ElevenLabs()
...
@api_bp.route("/convert-voice/<engine>", methods=["POST"])
        ...
        if "audio" in request.files or test_file:
            ...
            if engine == "openvoicev1" or engine == "openvoicev2":
                ...
                client = OpenVoiceApiClient(
                    base_url=app.config.get("OPENVOICE_API_URL"),
...
def health_check():
    ...
    if open_voice_server:
        try:
            response = requests.get(app.config.get("OPENVOICE_API_URL"))
    ...
    if fw_server:
        try:
            response = requests.get(app.config.get("FAST_WHISPER_API_URL"))
            response.raise_for_status()  # Raise exception for bad responses (4xx or 5xx)
            data = response.json()
            response_message += (
...
@api_bp.route("/concatenate-audio", methods=["POST"])
def concatenate_audio():
    try:
        ...
        output_dir = os.path.join(app.get("UPLOAD_FOLDER"), random_folder_name)
...
def call():
    ...
    url = (
        f"{app.config.get('EXTERNAL_ADDRESS')}/api/handle-call?start_conversation=1&"
    ...
    call = client.calls.create(
        ...
        status_callback=f"{app.config.get('EXTERNAL_ADDRESS')}/api/call-status",
...
@api_bp.route("/handle-call", methods=["GET", "POST"])
def handle_call():
...
@api_bp.route("/handle-speech", methods=["POST"])
def handle_speech():
...
@api_bp.route("/process-call", methods=["POST"])
def process_call():
    ...
    try:
        ...
        response = requests.post(
            app.config.get("EXTERNAL_ADDRESS") + "/api/chat/" + llmSource, json=params
        )
        data = response.json()
        elapsed_time = response.headers.get("X-Elapsed-Time")
        formatted_duration = f"{float(elapsed_time):.4f} ms"
        ...
        if data["result"]["data"]["response_message"]:
            ...
            response = requests.post(
                app.config.get("EXTERNAL_ADDRESS") + "/api/text-to-speech/" + ttsSource,
                json=params,
            )
            elapsed_time = response.headers.get("X-Elapsed-Time")
            formatted_duration = f"{float(elapsed_time):.4f} ms"
            ...
            if response.status_code == 200:
                ...
                response = VoiceResponse()
                response.play(
                    f"{app.config.get('EXTERNAL_ADDRESS')}/play-audio?file={output_file}"
                )
                response.redirect("/api/handle-call")
                return str(response)
